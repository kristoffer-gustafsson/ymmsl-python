...
def test_apply_custom_implementations(env_ymmsl_path: None, resolve: Resolve) -> None:
    ...
    assert (
        model.name == "a.g.a_e_test_model__customised_for__a_g_test_macro_micro_macro"
    )
...
def test_apply_custom_implementations_double_use(
    ...
    assert (
        model.name == "a.h.a_e_test_model__customised_for__a_h_test_macro_macro_macro2"
    )
...
def test_apply_custom_implementations_errors(
    ...
    config = load(ymmsl)
    assert isinstance(config, Configuration)
    config.custom_implementations[Reference("test_macro_mAcro.macro")] = Reference(
        "macro3"
    )
    ...
    config = load(ymmsl)
    assert isinstance(config, Configuration)
    config.custom_implementations[Reference("test_macro_macro.macro1[0]")] = Reference(
        "macro3"
    )
    with pytest.raises(RuntimeError):
        resolve(Reference("test_resolve_imports"), config)
    ...
    config = load(ymmsl)
    assert isinstance(config, Configuration)
    config.custom_implementations[Reference("test_macro_macro.macro3")] = Reference(
        "macro3"
    )
    with pytest.raises(RuntimeError):
        resolve(Reference("test_resolve_imports"), config)
...
def test_resolve_entrypoints_cached(mock_entry_points: Mock) -> None:
    config = load("""
        ymmsl_version: v0.2
        description: Test
        imports:
